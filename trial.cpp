    ...
    while (true) {
        ...
        if (cin >> choice) {
            if (choice >= 0 && choice < static_cast<int>(names.size())) {
                ...
                cout << "❌ Invalid choice. Please enter a number between 0 and " << names.size() - 1 << ".\n";
                clearInputBuffer();
            }
        } else {
...
void writeAttendanceReport(const std::vector<Student>& students) {
    try {
        std::ofstream out("attendance_report.txt");
        if (!out.is_open()) {
            throw std::ios_base::failure("Failed to open attendance report file for writing.");
        }
        ...
        for (const auto& s : students) {
            ...
            if (!out) {
                throw std::ios_base::failure("Failed to write to attendance report file.");
            }
        }
        out.close();
        if (out.fail()) {
            throw std::ios_base::failure("Failed to close attendance report file properly.");
        }
    }
    catch (const std::exception& e) {
        std::cerr << "Error writing attendance report: " << e.what() << std::endl;
    }
}
void clearInputBuffer() {
    char ch;
...
void markAttendance(std::vector<Student>& students) {
    try {
        ...
        for (auto& student : students) {
            int present;
            std::cout << "Is " << student.name << " present? (1/0): ";
            if (!(std::cin >> present)) {
                throw std::runtime_error("Invalid input: expected an integer.");
            }
            while (present != 0 && present != 1) {
                std::cout << "Invalid input. Enter 1 for present or 0 for absent: ";
                if (!(std::cin >> present)) {
                    throw std::runtime_error("Invalid input: expected an integer.");
                }
            }
            if (present == 1) {
                student.attendance = static_cast<float>((student.attendance + 1) / 101.0f) * 100.0f;
            } else {
                student.attendance = static_cast<float>((student.attendance) / 101.0f) * 100.0f;
            }
        }

        writeAttendanceReport(students);
        std::cout << "Attendance marked and report updated.\n";
    }
    catch (const std::exception& e) {
        std::cerr << "Error during attendance marking: " << e.what() << '\n';

        std::cin.clear();
        clearInputBuffer();  // Manually clear input buffer
    }
}
